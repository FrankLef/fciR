...
ggp_measures_modif <- function(df, title = "Change in effect measures",
  ...
  ggplot(df,
         aes(x = .data[["group"]], y = .data[["est"]],
             color = .data[["estimator"]], linetype = .data[["estimator"]],
             group = .data[["estimator"]])) +
    geom_line(size = 1) +
    geom_point(mapping = aes(fill = .data[["estimator"]]), size = 3, shape = 21) +
    geom_text(mapping = aes(label = round(.data[["est"]], 1)),
